...
namespace VMS.Controllers
{
    public class DriverChangeController : Controller
    {
        ...
        public ActionResult Create(DriverChangeModel model)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SP_InsertDriverChange", con))
                {
                    ...
                    cmd.Parameters.Add("@EndDate", SqlDbType.Date).Value = model.EndDate.HasValue ? model.EndDate.Value : (object)DBNull.Value;

                    cmd.Parameters.Add("@ReturnDate", SqlDbType.Date).Value = model.ReturnDate.HasValue ? model.ReturnDate.Value : (object)DBNull.Value;
        ...
        //               // ChangeDate = rdr["ChangeDate"] == DBNull.Value ? null : Convert.ToDateTime(rdr["ChangeDate"]).ToString("dd-MM-yyyy"),
        //                ChangeDate = rdr["ChangeDate"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(rdr["ChangeDate"]),
        ...
        //                EndDate = rdr["EndDate"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(rdr["EndDate"]),
            ...
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                ...
                while (rdr.Read())
                {
                    list.Add(new DriverChangeModel
                    {
                        ...
                        ChangeDate = rdr["ChangeDate"] == DBNull.Value
                            ? (DateTime?)null
                            : Convert.ToDateTime(rdr["ChangeDate"]),
                        ...
                        ReturnDate = rdr["ReturnDate"] == DBNull.Value
                            ? (DateTime?)null
                            : Convert.ToDateTime(rdr["ReturnDate"]),
                    });
                }
            }
            return Json(new { data = list });
